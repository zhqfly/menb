...
need = %w[Costs::Engine OpenProject::Reporting::Engine Budgets::Engine OpenProject::InternalExt::Engine]
missing = need - engines
puts "engines=#{engines.grep(/Cost|Report|Budget|Time|InternalExt/).join(",")}"
abort("MISSING engines: #{missing.join(",")}") unless missing.empty?
specs = Gem.loaded_specs.keys.grep(/cost|reporting|budget|internal_ext/)
...
%i[view_iext_evm manage_iext_cost_items view_iext_gantt_flow].each do |p|
  abort("MISSING plugin permission #{p}") unless perms.include?(p)
end
